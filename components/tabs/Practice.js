...
export default function Practice() {
  ...
  // Load subject progress from localStorage + Supabase
  useEffect(() => {
    if (!selectedExam) return;

    // First check localStorage
    const cachedProgress = localStorage.getItem(`progress_${selectedExam}`);
    if (cachedProgress) {
      ...
      try {
        ...
        if (user) {
          const { data } = await supabase
            .from('user_progress')
            .select('subject_id, correct_count, total_attempts')
            .eq('user_id', user.id)
            .eq('exam_id', userExams[selectedExam]?.id);

          if (data) {
            const formattedProgress = data.reduce((acc, item) => {
              acc[item.subject_id] = {
            ...
            localStorage.setItem(`progress_${selectedExam}`, JSON.stringify(formattedProgress));
          }
        }
      } catch (error) {
  ...
  const maxAvailableQuestions = useMemo(() => {
    ...
    return selectedSubjects.reduce((total, subject) => {
      const subjectData = userExams[selectedExam].subjects.find(s => s.id === subject);
      return total + (subjectData?.questionCount || 0);
    }, 0);
  ...
  const handleStartPractice = () => {
    ...
    router.push(
      `/practicesession?exam=${selectedExam}&subjects=${selectedSubjects.join(',')}&count=${Math.min(questionCount, maxAvailableQuestions)}`
    );
  };
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex justify-between text-sm text-gray-600 mt-1">
                ...
                <span>{maxAvailableQuestions}</span>
              </div>
            </div>
          </div>
        </Card>
